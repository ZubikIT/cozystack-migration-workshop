...
Манифесты из `manifests/` применяются с вашей машины через `kubectl`. Перед apply
откройте файл, поправьте namespace, а в `03-app-vm.yaml` ещё вставьте
presigned-ссылку (её печатает `convert.sh`).

Скрипты из `scripts/` запускаются не у вас, а внутри VM. Открываете скрипт,
вписываете свои значения вместо `ВСТАВЬТЕ_...`, переносите в VM (зашли через
`virtctl console` или `ssh`, вставили в `nano`, сохранили) и запускаете `bash`.
